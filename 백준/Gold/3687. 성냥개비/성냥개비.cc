...
void min_cal() { // dp 계산
    ...
    for (int i = 9; i < 101; ++i) {
        ...
        // num에 저장된 8의 최소 수는 10이지만, 직접적으로 사용되지는 않음. 단지 맨 위 for 문에서 dp[8] 값을 삽입할 때만 사용됨
        // j = 8을 사용하게 되면, 하단에 dp[i - j] 부분에서 dp[1]이 나올 수 있는데, 성냥개비 1개로 만들 수 있는 수는 아예 없기 때문에 유효하지 않은 dp값임
